...
import { RollupStatus, formatWeekEndingLabel, type StateSummaryResponse } from "@repo/types";
...
function stateNudgeCopy(state: StateSummaryResponse) {
  const weekLabel = formatWeekEndingLabel(state.weekOf);
  const waitingZones = state.zones.filter((zone) => !zone.forwarded).length;

  if (state.rollup?.status === RollupStatus.STALE) {
    return {
      pill: rollupLabel(RollupStatus.STALE),
      tone: rollupTone(RollupStatus.STALE),
      title: "Needs re-forward",
      meta: "A zone changed after you sent this week to HQ",
    };
  }
  ...
  if (waitingZones > 0) {
    return {
      ...
      title:
        waitingZones === 1
          ? "1 zone hasn't forwarded"
          : `${waitingZones} zones haven't forwarded`,
      meta: `Week ending ${weekLabel}`,
    };
  }

  if (state.rollup?.status === RollupStatus.FORWARDED) {
    return {
  ...
  return {
    ...
    meta: "Every zone has forwarded this week",
  };
}

export function StateActionNudge({
  state,
  onPress,
}: {
  ...
  const copy = stateNudgeCopy(state);
